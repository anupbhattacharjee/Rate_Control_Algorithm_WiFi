...
%% Transmit Rate control algorithm

%% Authers: Rahul Vyas, Anup kiran Bhattacharjee
...
avgg = (snr_p_n2 + snr_p_n1)/2;
   ...
   if isempty(y.EstimatedSNR)
    ...
    if (numPkt == i) % MCS determination based on fisrt packet SNR value
       snri = mean(y.EstimatedSNR); snrix = snri;
       if (snri >= 40) 
            MCS(numPkt) = 8;
            snrInd = 8;
       elseif (snri >= 25)
   ...
   delta = mean(y.EstimatedSNR) - snr_p;

   m = cfgVHT.MCS;

   if (ber(numPkt) == 0) % Packet not currupted
       
       if(abs(delta) > 1) % increasing SNR
           snrInd = snrInd +1;
       end  
       
       if (delta <0) %decreasing SNR
         if(abs(delta) > 1)
           snrInd = snrInd -2;
         end
       end
  
   else % Currupte Packet
       snrInd = snrInd -2;

   end 
 
    
     if (snrInd >= 9) % Boundary conditions
